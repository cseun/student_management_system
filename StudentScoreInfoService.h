...
public:
	StudentScoreInfoService(
		...
		StudentService& studentService,
		StudentScoreService& scoreService)
		: examInfoService(examInfoService), studentService(studentService), scoreService(scoreService) {}
	...
	for (auto& pair : allStudents)
	{
		...
		if (scoreIt != allScores.end())
		{
			...
			for (StudentScore& score : scores)
			{
				...
				studentScoreInfos.emplace_back(student, score, exam, totalScore);
			}
		}
	}
...
std::vector<StudentScoreInfo> StudentScoreInfoService::searchStudentScoreInfos(StudentListKey& listKey)
{
	...
	for (StudentScore& score : *scores)
	{
		...
		studentScoreInfos.emplace_back(searchedStudent, score, *examInfo, totalScore);
	}
